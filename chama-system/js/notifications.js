async function loadNotifications() {
  ...
  try {
    ...
    const items = notifs.slice(0, 10).map((n) => {
      ...
      return `
        <li class="bg-gray-50 border border-gray-200 rounded p-3">
          <div class="font-semibold text-gray-900 text-sm">${escapeHtml(title)}</div>
          <div class="text-gray-700 text-sm mt-1">${escapeHtml(msg)}</div>
    ...
    // Badge shows number of returned notifications (not unread; we don’t have read-tracking UI yet)
